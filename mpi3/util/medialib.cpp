...
void Mpi3Library::save(const QString &path){
    ...
    if(saveFile.open(QIODevice::ReadWrite | QIODevice::Truncate | QIODevice::Text)){
        ...
        for(int i = 0; i < this->libSongs->size(); i++){
            Mpi3Song *s = this->libSongs->at(i);
        ...
        for(int i = 0; i < this->libPlaylists->size(); i++){
            Mpi3Playlist *p = this->libPlaylists->at(i);
        ...
        for(int i = 0; i < this->libFolders->size(); i++){
            Mpi3Folder *f = this->libFolders->at(i);
...
Mpi3Playlist* Mpi3Library::newPlaylist(Mpi3Folder *parent){
    ...
    emit playlistInserted(playlist, parent, 0);
    return playlist;
}
Mpi3Folder* Mpi3Library::newFolder(Mpi3Folder *parent){
    ...
    emit folderInserted(folder, parent, 0);
    return folder;
}
...
void Mpi3Library::update(Mpi3Playlist *playlist){
    emit playlistUpdated(playlist);
}
void Mpi3Library::update(Mpi3Folder *folder){
...
void Mpi3Library::insert(Mpi3Song *inSong, Mpi3Playlist *toPlaylist, int position){
    ...
    emit songInserted(inSong, toPlaylist, position);
}
